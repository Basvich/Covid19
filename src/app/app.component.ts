...
import {
  IHumanOpt, Human, HumanFactory, IInfectionOptions, HStatus, getSuccess,
  getRndPopulationPos, getConsecutivePopulationPos, PopulationDistribution, getOrganicPopulationPos, getRndGauPos
} from './su-vir/ihuman';
...
export class AppComponent implements OnInit {
  ...
  public setupHumans() {
    ...
    this.posIndex=0;
    this.sqrDistanceBase = this.infectOp.distanceBase ** 2;
    this.mediumDistance = Math.sqrt(this.opt.zone.area() / this.populationNumber);
    this.humans = HumanFactory.create(this.populationNumber, this.opt); // HumanFactory.createTest();
    if (!this.fDistribution) this.fDistribution = this.FuncsPopulates[0].f;
    const posIterator = this.fDistribution(this.opt.zone);// getRndPopulationPos(opt.zone);
    // this.humansPosition= this.humans.map((h) => ({data: h, point: posIterator.next().value as Point}));
    this.setupPositions(this.opt);
  ...
  protected colorFromHumanStatus(h: Human) {
    ...
    if (h.hstatus & HStatus.infected) {
      if (h.hstatus & HStatus.infectious){
        if(h.hstatus & HStatus.symptomatic) return [255, 0, 0, 255];  //Rojo
        return [221,160,221,255]; // Asintomático
      }
      return [255, 153, 51, 255]; // Incubando
    }
    if (h.hstatus === HStatus.inmune) return [0, 255, 0, 255];
    if (h.hstatus === HStatus.death) return [10, 10, 10, 255];
    return [0, 255, 0, 255]; // Nada de nada
  }
